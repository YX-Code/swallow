...
public abstract class ClassUtils {
	...
	public static Class<?>[] toClassArray( Collection<Class<?>> collection) {
		return (!CollUtil.isEmpty(collection) ? collection.toArray(EMPTY_CLASS_ARRAY) : EMPTY_CLASS_ARRAY);
	}
	...
	public static String getShortNameAsProperty(Class<?> clazz) {
		...
		return uncapitalizeAsProperty(shortName);
	}
			...
			try {
				if (Modifier.isPublic(method.getModifiers())) {
					...
					Method specificMethod =
							ReflectUtil.getMethod(targetClass, method.getName(), method.getParameterTypes());
	 ...
	 */
	private static String uncapitalizeAsProperty(String str) {
		if (!StrUtil.isNotEmpty(str) || (str.length() > 1 && Character.isUpperCase(str.charAt(0)) &&
